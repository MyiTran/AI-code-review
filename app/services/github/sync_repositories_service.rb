module Github
  class SyncRepositoriesService < ApplicationService
    ...
    def remaining_repository_slots
      repository_count = Repository.cached_count(installation.user)
      repository_limit = Subscriptions::GetPlanLimitsService.call(installation.user).fetch(:repositories)
    ...
    def create_repository(github_repository)
      installation.repositories.create!(
        repository_attributes(github_repository).merge(connected: true, connected_at: Time.current, disconnected_at: nil)
      )
    end
